...
TFTPPACK rawToTFTP(char *buf, size_t packetLength) {
  TFTPPACK *packetptr = calloc(1, sizeof(TFTPPACK));
  ...
  *len += sizeof(short);
  ...
  case WRQ:
    *len += strlen(packet.stuff) + 1;
    raw = calloc(*len, 1);
    mempcpy(mempcpy(raw, &opcodeN, sizeof(short)), packet.stuff,
            strlen(packet.stuff) + 1);
...
void sendPacket(TFTPPACK packet, int sockfd, struct sockaddr_in sockClient) {
  ...
  sendto(sockfd, buff, packetSize, 0, (struct sockaddr *)&sockClient,
...
int main() {
  ...
  while (error == 0) {
    bzero(buf, BUFLEN);
    size_t packetLength;
    packetLength = recvfrom(sockfd, buf, BUFLEN, 0,
                            (struct sockaddr *)&sockClient, (socklen_t *)&slen);
    if (packetLength > 0) {
      system("clear");
